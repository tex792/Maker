...
void loop()
{
  ...
  {
    char c = XBee.read();
    //switch will take the character 'c', see which case applies, and then run the applicable code module. A 'break' statement needs to be used to break the loop.
    switch (c)
    {
...
void writeDPin()
{
  while (XBee.available() < 2); //The sketch will hold here until 2 bytes of data are available, pin and value
  char pin = XBee.read(); //Each time you tell the XBee to read, it will take in the next byte of data. In this case it's the pin number.
  char hl = ASCIItoHL(XBee.read()); //Read in the next byte of data, the desired value for the pin. Use the function 'ACIItoHL'
  //int hl = ASCIItoHL(XBee.read()); I tired using this line instead, and it will also work 

  // Print a message to let the control know of our intentions:
  XBee.print("Setting pin ");
  XBee.print(pin);
  XBee.print(" to ");
  XBee.println(hl ? "HIGH" : "LOW"); //I don't understand this syntax, left a comment on the Sparkfun XBee Shield tutorial to see if someone can explain
  //XBee.println(hl); I tired using this line instead, but it will not work

  //Now perform the action actually set the pin to the desired value
...
int ASCIItoHL(char c)
{
  ...
  if ((c == '0') || (c == 0) || (c == 'L') || (c == 'l'))
    ...
    return -1; //-1 is returned if no serial data is available
}

// ASCIItoInt
// Helper function to turn an ASCII hex value into a 0-15 byte val
//Reference an ASCII table to check the hex math shown below
